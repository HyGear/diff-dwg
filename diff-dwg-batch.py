# -*- coding: utf-8 -*-
"""
diff-dwg-batch.py: A hardcoded batch version of diff-dwg.py
"""
from Tkinter import *
...
global tempdir,size_check,olddir,newdir,diffdir,workfiles
#Change this if needed
tempdir = 'c:\\temp\\diff-dwg\\'
olddir = 'c:\\temp\\test\\old\\'
newdir = 'c:\\temp\\test\\new\\'
diffdir = 'c:\\temp\\test\\diff\\'
workfiles = [f for f in listdir(olddir) if isfile(join(olddir,f))]
size_check = 0
#Anaglyph matrices
...
def anaglyph(image1, image2, method=color_anaglyph):
...
def process_images():
    ...
    for work_f in workfiles:
        print "Old file:", work_f
        newfile=work_f.replace("old","new")
        print "New file:", newfile
        #    start = timeit.default_timer()
        filestr1=olddir + work_f
        filestr2=newdir + newfile
        filestr3=diffdir + work_f
        img1_file = pdf2jpg(filestr1, tempdir)
        img2_file = pdf2jpg(filestr2, tempdir)
        im1, im2 = Image.open(img2_file), Image.open(img1_file)
        #file_string = os.path.splitext(os.path.basename(filePath1))[0] + "-diff.jpg"
        diff_file = os.path.splitext(os.path.basename(filestr3))[0] + "-diff.jpg"
...
def Main():
    t1=threading.Thread(target=process_images)
    t1.start()
    #process_images()
#    global v
#    root = Tk()
